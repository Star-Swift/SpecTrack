...
CONFIG="${CONFIG:-spectrack_l224_must_ihmoe}"
...
checks = {
    ...
    "pretrained_large": Path("pretrained/itpn/fast_itpn_large_1600e_1k.pt").is_file(),
}
...
case "${MODE}" in
  ...
  eval)
    ...
    ;;
  *)
    echo "Usage: bash tracking/run_spectrack_l224.sh [check|train|eval]" >&2
    exit 2
    ;;
esac
