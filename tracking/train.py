...
def main():
    args = parse_args()
    lock_dir = os.path.join(os.path.abspath(args.save_dir or "."), "logs", "train_locks")
    os.makedirs(lock_dir, exist_ok=True)
    lock_path = os.path.join(lock_dir, "%s-%s.lock" % (args.script, args.config))
    print("[LOCK] waiting for %s" % lock_path, flush=True)
    with open(lock_path, "w") as lock_file:
        fcntl.flock(lock_file.fileno(), fcntl.LOCK_EX)
        lock_file.seek(0)
        lock_file.truncate()
        lock_file.write("pid=%s\n" % os.getpid())
        lock_file.flush()
        print("[LOCK] acquired %s" % lock_path, flush=True)
        _main_locked(args)
