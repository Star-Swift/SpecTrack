...
CONFIG="${CONFIG:-spectrack_b224_must_ihmoe}"
...
checks = {
    ...
    "pretrained_base": Path("pretrained/itpn/fast_itpn_base_clipl_e1600.pt").is_file(),
}
...
case "${MODE}" in
  ...
  eval)
    ...
    ;;
  *)
    echo "Usage: bash tracking/run_spectrack_b224.sh [check|train|eval]" >&2
    exit 2
    ;;
esac
