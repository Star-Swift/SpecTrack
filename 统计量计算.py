...
# 原始默认导入
from lib.train.dataset.MSITrack import MSITrack
...
def get_frame_path_for_seq(seq_path, frame_id):
	...
	return os.path.join(seq_path, name_list[frame_id] + '.mat')
...
def main():
	parser = argparse.ArgumentParser(description="计算 MSITrack 等多光谱数据集每通道均值与标准差（流式处理版）")
	parser.add_argument('--root', type=str, default='/root/autodl-tmp/SUTrack-moe/data/MSITrack', help='数据集根目录')
	parser.add_argument('--dataset', type=str, default='lib.train.dataset.MSITrack.MSITrack', help='数据集类完整路径')
	parser.add_argument('--image_loader', type=str, default='lib.train.data.image_loader.msi_loader', help='image_loader 导入路径')
