...
class Preprocessor(object):
    def __init__(self, cfg=None):
        ...
        self.cfg_mean = None
        self.cfg_std = None
        if cfg is not None and hasattr(cfg, "DATA") and hasattr(cfg.DATA, "MEAN") and hasattr(cfg.DATA, "STD"):
            self.cfg_mean = torch.tensor(cfg.DATA.MEAN).view((1, len(cfg.DATA.MEAN), 1, 1)).cuda()
            self.cfg_std = torch.tensor(cfg.DATA.STD).view((1, len(cfg.DATA.STD), 1, 1)).cuda()
...
0.291505,
0.336807
        ]).view((1, 16, 1, 1)).cuda()  # 形状: (1, 8, 1, 1)

        # 8通道的标准差
        self.gp_std = torch.tensor([
0.173026,
0.179336,
0.173548,
0.180468,
...
0.255125
    ...
    def process(self, img_arr: np.ndarray):
        ...
        if self.cfg_mean is not None and self.cfg_mean.size(1) == channels:
            mean = self.cfg_mean
            std = self.cfg_std
        elif img_arr.shape[-1] == 6:
            mean = self.mm_mean
            std = self.mm_std
        elif img_arr.shape[-1] == 16:
            mean = self.gp_mean
            std = self.gp_std
        else:
            mean = self.mean
