...
class SUTRACK(BaseTracker):
    def __init__(self, params, dataset_name):
        ...
        # checkpoint 路径（允许用环境变量覆盖，便于批量测试）
        default_ckpt = '/root/autodl-tmp/SUTrack-moe/checkpoints/train/sutrack/sutrack_b224_must_ihmoe_32_4_2_6/SUTRACK_ep0040.pth.tar'
        env_ckpt = os.environ.get('SUTRACK_CHECKPOINT', '').strip()
        self.params.checkpoint = env_ckpt or getattr(self.params, 'checkpoint', None) or default_ckpt
    ...
    def track(self, image, info: dict = None):
        ...
        #     if 'moce' in name and block_index != name.split('.')[0]:
        #         block_index = name.split('.')[0]
        #         self.routing_info.append(getattr(blocks, block_index).moce.routing_info)
