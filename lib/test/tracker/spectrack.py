...
from lib.models.spectrack import build_spectrack
...
class SPECTRACK(BaseTracker):
    def __init__(self, params, dataset_name):
        super(SPECTRACK, self).__init__(params)
        network = build_spectrack(params.cfg)

        # checkpoint path can be overridden by SPECTRACK_CHECKPOINT for batch evaluation.
        env_ckpt = os.environ.get('SPECTRACK_CHECKPOINT', '').strip()
        self.params.checkpoint = env_ckpt or getattr(self.params, 'checkpoint', None)
        if not self.params.checkpoint:
            raise FileNotFoundError("Checkpoint is not set. Set SPECTRACK_CHECKPOINT or params.checkpoint.")
        if not os.path.isfile(self.params.checkpoint):
            raise FileNotFoundError(self.params.checkpoint)

        # print("Loading checkpoint:", self.params.checkpoint)
    ...
    def initialize(self, image, info: dict):
        ...
        #     else:
        #         init_nlp = None
        #     text_data, _ = self.prepare_text_tokens(init_nlp)
    ...
    def track(self, image, info: dict = None):
        ...
        # Expert routing trace for MoCE diagnostics.
        self.routing_info = []
        blocks = self.network.encoder.body.blocks
        for block_idx, block in enumerate(blocks):
            if getattr(block, "use_moce", False) and hasattr(block, "moce"):
                info = getattr(block.moce, "routing_info", None)
                if info:
                    info = dict(info)
                    info["block_idx"] = block_idx
                    self.routing_info.append(info)
    ...
    def prepare_text_tokens(self, nlp):
        seq_len = getattr(self, "text_seq_len", 77)
        ids, masks = self._normalize_text_tokens(nlp, seq_len)
        return ids, masks

    def _normalize_text_tokens(self, nlp, seq_len):
        ids, masks = None, None
        ...
        def tensorize(value):
            ...
            if isinstance(value, torch.Tensor):
                return value.detach().clone().to(dtype=torch.long).flatten()
            if isinstance(value, np.ndarray):
                return torch.from_numpy(value).to(dtype=torch.long).flatten()
            return torch.as_tensor(value, dtype=torch.long).flatten()

        def pad_or_trim(tensor):
            if tensor.numel() >= seq_len:
                return tensor[:seq_len]
            pad = torch.zeros(seq_len - tensor.numel(), dtype=tensor.dtype)
            return torch.cat([tensor, pad], dim=0)

        if nlp is None:
            ids = torch.zeros(seq_len, dtype=torch.long)
        elif isinstance(nlp, dict):
            ids = nlp.get("ids") or nlp.get("input_ids")
            masks = nlp.get("masks") or nlp.get("attention_mask")
        elif isinstance(nlp, (tuple, list)) and len(nlp) == 2:
            ids, masks = nlp
        elif isinstance(nlp, (torch.Tensor, np.ndarray, list, tuple)):
            ids = nlp
        elif isinstance(nlp, str):
            warnings.warn("CLIP tokenizer 不可用，忽略原始字符串文本输入。", RuntimeWarning)
            ids = torch.zeros(seq_len, dtype=torch.long)
        else:
            warnings.warn("未识别的文本输入类型，使用零向量。", RuntimeWarning)
            ids = torch.zeros(seq_len, dtype=torch.long)

        ids = pad_or_trim(tensorize(ids))
        if masks is None:
            masks = (ids == 0).long()
        else:
            masks = pad_or_trim(tensorize(masks))
