...
def run_sequence(seq: Sequence, tracker: Tracker, debug=False, num_gpu=8):
    ...
    print(len(tracker.full_routing_info))
    os.makedirs('moeeval', exist_ok=True)
    run_part = "none" if tracker.run_id is None else str(tracker.run_id)
    routing_name = (
        f"routing_info_{tracker.name}_{tracker.parameter_name}_"
        f"run{run_part}_epoch{tracker.epoch}_{seq.name}.pkl"
    )
    with open(os.path.join("moeeval", routing_name), 'wb') as f:
        pickle.dump(tracker.full_routing_info, f)
