...
class MSITrackDataset(BaseDataset):
    """MSITrack dataset loader for evaluation."""
    def __init__(self, split='test'):
        super().__init__()
        base_dir = getattr(self.env_settings, 'msitrack_path', None) or getattr(self.env_settings, 'msitrack_dir', None)
        if base_dir is None:
            raise ValueError("MSITrack path is not set in env settings.")
        self.split = split
    ...
    def _construct_sequence(self, sequence_name):
        ...
        frames_list = [os.path.join(seq_path, f"{name}.mat") for name in frame_names]

        target_visible = np.ones(len(frames_list), dtype=np.bool_)
        obj_class = self._infer_class_name(sequence_name)

        return Sequence(sequence_name, frames_list, 'msitrack', ground_truth_rect,
