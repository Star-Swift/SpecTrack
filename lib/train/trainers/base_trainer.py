...
class BaseTrainer:
        ...
        for i in range(num_tries):
            try:
                ...
                for epoch in range(self.epoch+1, max_epochs+1):
                    ...
                    save_last_n = getattr(self.settings, "save_last_n", 10)
                    save_every_epoch = getattr(self.settings, "save_every_epoch", False)
                    save_every_n = getattr(self.settings, "save_every_n", 10)
                    in_last_n = save_last_n > 0 and epoch > (max_epochs - save_last_n)
                    periodic_save = save_every_n > 0 and epoch % save_every_n == 0
                    if in_last_n or save_every_epoch or periodic_save:
    ...
    def save_checkpoint(self):
        ...
        self.prune_old_checkpoints(directory, net_type)

    def prune_old_checkpoints(self, directory, net_type):
        save_last_n = getattr(self.settings, "save_last_n", 10)
        save_every_n = getattr(self.settings, "save_every_n", 10)
        if save_last_n <= 0 or save_every_n > 0:
            return
        checkpoint_list = sorted(glob.glob('{}/{}_ep*.pth.tar'.format(directory, net_type)))
        for checkpoint_path in checkpoint_list[:-save_last_n]:
