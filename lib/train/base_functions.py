...
def update_settings(settings, cfg):
    ...
    settings.save_last_n = getattr(cfg.TRAIN, "SAVE_LAST_N", 10)
    settings.save_every_epoch = getattr(cfg.TRAIN, "SAVE_EVERY_EPOCH", False)
    settings.save_every_n = getattr(cfg.TRAIN, "SAVE_EVERY_N", 10)
    settings.non_blocking_transfer = getattr(cfg.TRAIN, "NON_BLOCKING_TRANSFER", True)
...
def names2datasets(name_list: list, settings, image_loader):
    ...
    for name in name_list:
        ...
        if name == "HOT2022":
            datasets.append(HOT2022(root=settings.env.hot2022_dir, split='train'))
        if name == "MUSTHSI":
...
def build_dataloaders(cfg, settings):
    os.environ.setdefault("SPECTRACK_MUST_MMAP_CACHE_SIZE", str(getattr(cfg.TRAIN, "MUST_MMAP_CACHE_SIZE", 128)))
    ...
    loader_train = LTRLoader('train', dataset_train, training=True, batch_size=cfg.TRAIN.BATCH_SIZE, shuffle=shuffle,
                             ...
                             pin_memory=getattr(cfg.TRAIN, "PIN_MEMORY", True) and torch.cuda.is_available(),
                             persistent_workers=num_workers > 0 and getattr(cfg.TRAIN, "PERSISTENT_WORKERS", True),
