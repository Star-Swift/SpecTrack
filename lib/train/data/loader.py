...
def ltr_collate_stack1(batch):
    ...
    if isinstance(batch[0], torch.Tensor):
        ...
        if _check_use_shared_memory():
            ...
            out = batch[0].new(storage)
            for i in batch:
                if i.shape != batch[0].shape:
                    pass
        return torch.stack(batch, 1)  # Removed resize_ to avoid resizing issues
    elif elem_type.__module__ == 'numpy' and elem_type.__name__ != 'str_' \
            and elem_type.__name__ != 'string_':
        elem = batch[0]
        if elem_type.__name__ == 'ndarray':
...
class LTRLoader(torch.utils.data.dataloader.DataLoader):
    ...
    def __init__(self, name, dataset, training=True, batch_size=1, shuffle=False, sampler=None, batch_sampler=None,
        ...
        loader_kwargs = dict(dataset=dataset, batch_size=batch_size, shuffle=shuffle, sampler=sampler,
                             batch_sampler=batch_sampler, num_workers=num_workers, collate_fn=collate_fn,
                             pin_memory=pin_memory, drop_last=drop_last, timeout=timeout,
                             worker_init_fn=worker_init_fn, pin_memory_device=pin_memory_device,
                             in_order=in_order)
        if num_workers > 0:
        ...
        super(LTRLoader, self).__init__(**loader_kwargs)
