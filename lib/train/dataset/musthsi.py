...
class MUSTHSI(BaseVideoDataset):
    ...
    def get_sequence_info(self, seq_id):
        ...
        if seq_id in self._sequence_info_cache:
            return self._sequence_info_cache[seq_id]
        ...
        seq_info = {'bbox': bbox, 'valid': valid, 'visible': visible}
        self._sequence_info_cache[seq_id] = seq_info
        return seq_info

    def _get_frame_path(self, seq_path, frame_id):
        # 获取指定帧的图像路径
        name_list = self._get_frame_names(seq_path)
    ...
    def _get_frame_names(self, seq_path):
        name_list = self._frame_name_cache.get(seq_path)
        if name_list is None:
            # full_occlusion.txt contains frame file names; cache it per worker.
            with open(os.path.join(seq_path, "full_occlusion.txt")) as f:
                name_list = [line.strip().split(',')[0] for line in f]
            self._frame_name_cache[seq_path] = name_list
        return name_list
