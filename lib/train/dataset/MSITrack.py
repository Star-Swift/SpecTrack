...
from lib.train.data import msi_loader
from lib.train.admin import env_settings


class MSITrack(BaseVideoDataset):
    def __init__(self, root=None, image_loader=msi_loader, split=None, seq_ids=None, data_fraction=None):
        """
        args:
            root - 数据集路径
        ...
        """
        root = env_settings().hsitrack_dir if root is None else root
        super().__init__('MSITrack', root, image_loader)
    ...
    def _get_frame_path(self, seq_path, frame_id):
        ...
        name_path = os.path.join(seq_path, name_list[frame_id] + '.mat')    # frames start from 1
