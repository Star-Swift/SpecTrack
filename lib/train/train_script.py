...
from lib.train.actors import SpecTrack_Actor
...
def run(settings):
    settings.description = 'Training script for SpecTrack'
    ...
    # Create network
    if settings.script_name == "spectrack":
        net = build_spectrack(cfg)
    else:
    ...
    #         print(f"{name}: {param.is_contiguous()}")


    # wrap networks to distributed one
    net.cuda()
    if settings.local_rank != -1:
        net = DDP(net, broadcast_buffers=False, device_ids=[settings.local_rank], find_unused_parameters=False) # modify the find_unused_parameters to False to skip a runtime error of twice variable ready
    ...
    if  settings.script_name == "spectrack":
        ...
        actor = SpecTrack_Actor(net=net, objective=objective, loss_weight=loss_weight, settings=settings, cfg=cfg)
        actor.training = True
    else:
        raise ValueError("illegal script name")
