...
def spectral_gradient_energy(x):
    """Channel-axis high-frequency proxy for latent MSI/HSI features.

    The input has already passed patch embedding, so the channel axis is not a
    calibrated wavelength axis. This branch is therefore used as a spectral
    continuity proxy for routing, while raw-band prompts can be wired later.
    """
    ...
    return first.abs() + 0.5 * second.abs()


def raw_band_gradient_prompt(raw, dim):
    """Project raw-band first/second differences to the latent prompt width."""
    prompt = spectral_gradient_energy(raw).mean(dim=(-2, -1))
    if prompt.size(1) == dim:
        return prompt
    if prompt.size(1) == 1:
        return prompt.expand(-1, dim)
    return F.interpolate(prompt.unsqueeze(1), size=dim, mode="linear", align_corners=False).squeeze(1)


class FrequencyEmbedding(nn.Module):
    """Frequency prompt for routing.

    ``legacy`` preserves the original spatial high-pass global embedding.
    Other modes add channel-axis spectral-frequency proxies and/or a separable
    spatial-spectral branch for HSI/MSI-specific routing ablations.
    """
    def __init__(self, dim, mode="spatial_spectral", spectral_permute_seed=-1, spectral_source="latent"):
        super(FrequencyEmbedding, self).__init__()
        valid_modes = {"legacy", "spatial", "spectral", "spatial_spectral", "spatial_spectral_joint"}
        if mode not in valid_modes:
            raise ValueError(f"mode must be one of {sorted(valid_modes)}")
        if spectral_source not in {"latent", "raw_band"}:
            raise ValueError("spectral_source must be 'latent' or 'raw_band'")
        self.mode = mode
        self.spectral_source = spectral_source
        self.spectral_permute_seed = int(spectral_permute_seed)
        if self.spectral_permute_seed == -2:
            perm = torch.arange(dim - 1, -1, -1)
            self.register_buffer("spectral_perm", perm, persistent=False)
        elif self.spectral_permute_seed >= 0:
            generator = torch.Generator()
            generator.manual_seed(self.spectral_permute_seed)
            perm = torch.randperm(dim, generator=generator)
            self.register_buffer("spectral_perm", perm, persistent=False)
        else:
            self.spectral_perm = None
        self.high_conv = nn.Sequential(HighPassConv2d(dim, freeze=True), nn.GELU())
        in_dim = dim if mode in {"legacy", "spatial", "spectral"} else 2 * dim
        if mode == "spatial_spectral_joint":
            in_dim = 3 * dim
        self.mlp = nn.Sequential(nn.Linear(in_dim, 2 * dim), nn.GELU(), nn.Linear(2 * dim, dim))

    def _spectral_input(self, x):
        if self.spectral_perm is None:
            return x
        return x.index_select(1, self.spectral_perm.to(x.device))

    def forward(self, x, raw_spectral_prompt=None):
        if self.mode in {"legacy", "spatial"}:
            prompt = self.high_conv(x).mean(dim=(-2, -1))
        elif self.mode == "spectral":
            if self.spectral_source == "raw_band" and raw_spectral_prompt is not None:
                prompt = raw_spectral_prompt
            else:
                prompt = spectral_gradient_energy(self._spectral_input(x)).mean(dim=(-2, -1))
        else:
            spatial_prompt = self.high_conv(x).mean(dim=(-2, -1))
            if self.spectral_source == "raw_band" and raw_spectral_prompt is not None:
                spectral_prompt = raw_spectral_prompt
            else:
                spectral_prompt = spectral_gradient_energy(self._spectral_input(x)).mean(dim=(-2, -1))
            prompts = [spatial_prompt, spectral_prompt]
            if self.mode == "spatial_spectral_joint":
                if self.spectral_source == "raw_band" and raw_spectral_prompt is not None:
                    joint_prompt = raw_spectral_prompt
                else:
                    joint_prompt = spectral_gradient_energy(self._spectral_input(self.high_conv(x))).mean(dim=(-2, -1))
                prompts.append(joint_prompt)
            prompt = torch.cat(prompts, dim=-1)
        return self.mlp(prompt)


class SpectralGlobalExpert(nn.Module):
    """Shared global context branch with spectral-continuity-aware modulation."""
    def __init__(self, dim, num_heads=8):
        super().__init__()
        self.shared_attn = SharedAttention(dim, num_heads=num_heads)
        hidden = max(16, dim // 4)
    ...
    def forward(self, x):
        shared = self.shared_attn(x)
        pooled = x.mean(dim=(-2, -1))
        spectral_energy = spectral_gradient_energy(x).mean(dim=(-2, -1))
        gate = self.context_gate(torch.cat([pooled, spectral_energy], dim=-1)).unsqueeze(-1).unsqueeze(-1)
        return self.project_out(shared * gate + x * (1.0 - gate))



class RoutingFunction(nn.Module):
    """路由函数：生成门控分数，选择激活的专家"""
    def __init__(self, dim, freq_dim, num_experts, k, complexity, use_freq=True,
                 use_complexity_bias=True, complexity_scale="max", noise_eval=True):
        ...
        self.freq_gate = nn.Linear(freq_dim, num_experts, bias=False) if use_freq else None
        if complexity_scale == "min":
            complexity = complexity / complexity.min()
        elif complexity_scale == "max":
            complexity = complexity / complexity.max()
        else:
            raise ValueError("complexity_scale must be 'max' or 'min'")
    ...
    def forward(self, x, freq_emb):
        logits = self.gate(x)
        if self.use_freq:
            logits = logits + self.freq_gate(freq_emb)
        aux_loss = 0
        if self.training:
            aux_loss = self.importance_loss(logits.softmax(dim=-1))

        if self.training or self.noise_eval:
...
class MoCEAdapter(nn.Module):
    ...
    核心创新：
        ...
        3. 稀疏激活：仅激活top-k专家
    """
    def __init__(self, dim: int, rank: int = 64, rank_scaling: str = "constant",
                 num_experts: int = 4, top_k: int = 2,
                 expert_layer: nn.Module = FFTAttention, stage_depth: int = 1,
                 expert_type: str = "heterogeneous", depth_type: str = "constant", freq_dim: int = None,
                 use_freq: bool = True, use_shared: bool = True,
                 freq_mode: str = "spatial_spectral", shared_type: str = "attention",
                 with_complexity: bool = True, complexity_scale: str = "max",
                 spectral_permute_seed: int = -1, spectral_source: str = "latent",
                 routing_noise_eval: bool = True):
        super().__init__()
        ...
        if expert_type == "heterogeneous":
            ...
            if depth_type == "constant":
                ...
                raise ValueError("depth_type must be 'constant' or 'lin'")
        elif expert_type == "homogeneous":
            patch_sizes = [8 for _ in range(num_experts)]
            kernel_sizes = [5 for _ in range(num_experts)]
            depths = [stage_depth for _ in range(num_experts)]
        else:
            raise ValueError("expert_type must be 'heterogeneous' or 'homogeneous'")

        if expert_type == "homogeneous" or rank_scaling == "constant":
            ranks = [rank for _ in range(num_experts)]
        elif rank_scaling == "nested":
            ranks = [max(8, rank * (i + 1) // num_experts) for i in range(num_experts)]
        elif rank_scaling == "exponential":
            ranks = [max(8, rank // (2 ** (num_experts - i - 1))) for i in range(num_experts)]
        else:
            raise ValueError("rank_scaling must be 'constant', 'nested', or 'exponential'")

        self.freq_embed = FrequencyEmbedding(dim, mode=freq_mode,
                                             spectral_permute_seed=spectral_permute_seed,
                                             spectral_source=spectral_source) if use_freq else None
        if use_shared:
            if shared_type == "attention":
                self.shared_attn = SharedAttention(dim, num_heads=max(1, dim // 64))
            elif shared_type == "spectral_global":
                self.shared_attn = SpectralGlobalExpert(dim, num_heads=max(1, dim // 64))
            else:
                raise ValueError("shared_type must be 'attention' or 'spectral_global'")
        else:
        ...
        self.expert_param_counts = [int(v.item()) for v in expert_complexity]
        self.routing = RoutingFunction(dim, freq_dim, num_experts=num_experts, k=top_k,
                                        ...
                                        noise_eval=routing_noise_eval)
        self.routing_info = {}

    def forward(self, x, H, W, raw_spectral_prompt=None):
        """
        ...
        freq_emb = self.freq_embed(x_4d, raw_spectral_prompt) if self.use_freq else None
        shared = self.shared_attn(x_4d) if self.use_shared else x_4d
        ...
        if not self.training:
            self.routing_info = {
                ...
                "top_k_values": top_k_values.detach().cpu().tolist(),
                "expert_param_counts": self.expert_param_counts,
            }
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 ...
                 use_moce=False,
                 moce_rank=64,
                 moce_rank_scaling="constant",
                 moce_num_experts=4,
                 moce_top_k=2,
                 moce_expert_type="heterogeneous",
                 moce_depth_type="constant",
                 moce_use_freq=True,
                 moce_use_shared=True,
                 moce_freq_mode="spatial_spectral",
                 moce_shared_type="attention",
                 moce_use_complexity_bias=True,
                 moce_complexity_scale="max",
                 moce_spectral_permute_seed=-1,
                 moce_spectral_source="latent",
                 moce_routing_noise_eval=True,
        ...
        if self.use_moce:
            self.moce = MoCEAdapter(
                dim=dim,
                rank=moce_rank,
                rank_scaling=moce_rank_scaling,
                num_experts=moce_num_experts,
                top_k=moce_top_k,
                expert_type=moce_expert_type,
                depth_type=moce_depth_type,
                use_freq=moce_use_freq,
                use_shared=moce_use_shared,
                freq_mode=moce_freq_mode,
                shared_type=moce_shared_type,
                with_complexity=moce_use_complexity_bias,
                complexity_scale=moce_complexity_scale,
                spectral_permute_seed=moce_spectral_permute_seed,
                spectral_source=moce_spectral_source,
                routing_noise_eval=moce_routing_noise_eval,
            )
            self.moce_scale = nn.Parameter(torch.zeros(1))  # 可学习缩放因子，初始为0
        self.raw_spectral_prompt = None
        ...
        if self.gamma_2 is None:
            if self.postnorm:
                ...
                if self.use_moce:
                    ...
                    moce_out = self.moce(x_search, H_search, W_search, self.raw_spectral_prompt)
...
class Fast_iTPN(nn.Module):
    def __init__(self, search_size=224,template_size=112, patch_size=16, in_chans=8, embed_dim=512, depth_stage1=3, depth_stage2=3, depth=24,
        ...
        self.moce_rank_scaling = moce_rank_scaling
        self.moce_num_experts = moce_num_experts
        self.moce_top_k = moce_top_k
        self.moce_start_layer = moce_start_layer
        self.moce_expert_type = moce_expert_type
        self.moce_depth_type = moce_depth_type
        self.moce_use_freq = moce_use_freq
        self.moce_use_shared = moce_use_shared
        self.moce_freq_mode = moce_freq_mode
        self.moce_shared_type = moce_shared_type
        self.moce_use_complexity_bias = moce_use_complexity_bias
        self.moce_complexity_scale = moce_complexity_scale
        self.moce_spectral_permute_seed = moce_spectral_permute_seed
        self.moce_spectral_source = moce_spectral_source
        self.moce_routing_noise_eval = moce_routing_noise_eval
        ...
        for layer_idx in range(depths[2]):
            ...
            self.blocks.append(
                Block(
                    ...
                    moce_rank_scaling=self.moce_rank_scaling,
                    moce_num_experts=self.moce_num_experts,
                    moce_top_k=self.moce_top_k,
                    moce_expert_type=self.moce_expert_type,
                    moce_depth_type=self.moce_depth_type,
                    moce_use_freq=self.moce_use_freq,
                    moce_use_shared=self.moce_use_shared,
                    moce_freq_mode=self.moce_freq_mode,
                    moce_shared_type=self.moce_shared_type,
                    moce_use_complexity_bias=self.moce_use_complexity_bias,
                    moce_complexity_scale=self.moce_complexity_scale,
                    moce_spectral_permute_seed=self.moce_spectral_permute_seed,
                    moce_spectral_source=self.moce_spectral_source,
                    moce_routing_noise_eval=self.moce_routing_noise_eval,
    ...
    def prepare_tokens_with_masks(self, template_list, search_list, template_anno_list, text_src, task_index):
        ...
        raw_spectral_prompt = None
        if self.moce_spectral_source == "raw_band":
            raw_spectral_prompt = raw_band_gradient_prompt(x, self.embed_dim)
            raw_spectral_prompt = raw_spectral_prompt.view(B, num_search, -1).mean(dim=1)
        self.raw_spectral_prompt = raw_spectral_prompt
    ...
    def forward_features(self, template_list, search_list,template_anno_list, text_src, task_index):
        ...
        for blk in self.blocks[-self.num_main_blocks:]:
            if getattr(blk, "use_moce", False):
                blk.raw_spectral_prompt = getattr(self, "raw_spectral_prompt", None)
